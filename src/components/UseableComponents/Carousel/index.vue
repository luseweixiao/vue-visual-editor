<template>
  <div id="swiper"
    ...
    <div class="swiper-box"
         :style="{width:allCount,height:imgHeight+'px','-webkit-transition':transitionConfig,'-webkit-transform':slateX}"
         ref='swiper'>
      <carousel-item :img="imgList[imgList.length-1]"
                     :imgWidth="-imgWidth"
                     :imgH="imgHeight"
                     :imgW="imgWidth"
                     v-if="loop" />
      <carousel-item v-for="(item,index)  in imgList"
                     :key="index"
                     :img="item"
                     :imgH="imgHeight"
                     :imgW="imgWidth"
                     :imgWidth="imgWidth*index"></carousel-item>
      <carousel-item :img="imgList[0]"
                     :imgWidth="imgWidth*imgList.length"
                     v-if="loop"
                     :imgH="imgHeight"
                     :imgW="imgWidth"></carousel-item>
    </div>
    <div class="swiper-dots"
         v-if="dots">
      <button v-for="(item,index) in imgList.length"
              :key="index"
